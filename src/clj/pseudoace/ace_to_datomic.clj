...
(defn generate-schema [options]
  ...
  (let [ models (model/parse-models (io/reader (string/join "" ["models/" (:model options)])))]
...
(defn acedump-to-datomic-log [options]
  ...
  (let [uri (:url options)
       con (datomic/connect uri)
       imp (old-import/importer con) ;; Helper object, holds a cache of schema data.
       log-dir (io/file (:log-dir options))   ;; Must be an empty directory
...
(defn write-datomic-database-report [filename db]
  (let [elements-attributes (sort (datomic/q '[:find [?ident ...] :where [_ :db/ident ?ident]] db))]
...
(defn -main [& args]
  ...
  (case (last arguments)
    ...
    (if (or (string/blank? (:url options))
      ...
      (println "The options url and datomic-database-report-filename are required for generating the report")
